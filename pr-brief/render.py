...
class Brief:
    ...
    @staticmethod
    def line_attrs(block, path, number):
        """Anchor of a code line for the notes. A line of another repo (the agorize-core
        companion) stays annotable but carries no path/line: it is not a line of this PR."""
        if block.get("repo"):
            return f' data-anchor="c:{esc(block["repo"])}:{esc(path)}:{number}"'
        return (f' data-anchor="c:{esc(path)}:{number}" data-path="{esc(path)}"'
                f' data-line="{number}" data-side="RIGHT"')

    def render_diff(self, block):
        path = str(block.get("path", ""))
        # a diff from another PR (the agorize-core companion) carries its own `repo` and `url`
        foreign = bool(block.get("repo"))
        rows = []
        ...
        for kind, old_n, new_n, text in rows:
            ...
            if kind in ("add", "ctx") and new_n:
                attrs += self.line_attrs(block, path, new_n)
            cells.append(
        ...
        return (
            '<div class="code">'
            + self.file_header(f"{block['repo']} — {path}" if foreign else path,
                               block.get("url") or (None if foreign else self.diff_link(path)))
    ...
    def render_context(self, block):
        ...
        for index, segment in enumerate(segments):
            ...
            for offset, text in enumerate(str(segment.get("code", "")).split("\n")):
                ...
                lines += (f'<tr{cls}{self.line_attrs(block, path, number)}>'
