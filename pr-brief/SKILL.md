...
- **The ref is shared between passes** (`pr-brief`, `pr-flow`, `review-requested-prs` often run
  one after the other, or side by side, on the same PR). Before fetching, check it:
  `git rev-parse -q --verify refs/pr-brief/<N>`. If it exists and equals the PR's head sha, reuse
  it and **do not delete it at the end** — another pass owns it. If it exists at another sha, it is
  stale: re-fetch it (`git fetch origin +refs/pull/<N>/head:refs/pr-brief/<N>`). Delete at the end
  only the refs this run created: `git update-ref -d refs/pr-brief/<N>`. When the invocation says
  another pass follows, keep them and say so in the report.
- **Shell traps.** In zsh, `$R:src/x.vue` applies the `:s` modifier to `$R` — write `${R}:src/x.vue`.
  The rtk hook rewrites `git show <ref>:<path>` into a commit summary: read a file from the shell
  with `git cat-file -p <ref>:<path>` (`blocks.py` already does).
- **Look for a local clone before cloning.** Match on the remote, not on the directory name — a
  ...
  two levels under the projects root, `~/workspaces` (the Agorize repos live in
  `~/workspaces/agorize/<repo>`; `*.worktrees/` siblings are skipped by the `-d .git` test, since a
  worktree's `.git` is a file):
  ```
  for d in ~/workspaces/*/ ~/workspaces/*/*/; do
    [ -d "$d.git" ] && git -C "$d" remote get-url origin 2>/dev/null | grep -qiE '[:/]<OWNER>/<REPO>(\.git)?$' && echo "$d"
  done
  ```
  Several hits (a mirror, an `_integrated` or `-webpack` copy) → take the one whose directory
  name is exactly `<REPO>` (`~/workspaces/agorize/agorize-front`, not `agorize-front-webpack`). Reading through a
...
- **Never `gh pr checkout` and never `git checkout` in a local clone** — the main checkout reflects
  the user's current work, clean or not. When a tool genuinely needs the files on disk, create a
  sibling worktree on the named ref (`git worktree add --detach ../<repo>.worktrees/pr-brief-<N>
  refs/pr-brief/<N>`), never one inside the repo, and remove it when done
...
Several PRs → one subagent per PR. Reading through a named ref needs no worktree; never use
`isolation: "worktree"` (it creates the worktree inside the repo, which breaks Vitest and Vite) —
use the sibling worktree above if a PR forces files on disk. Each subagent writes its own JSON + HTML and
returns the output path. Leave the working tree as you found it.

## Step 3 — Write the brief as JSON

Write `<report>.json`, where `<report>` is the path printed by
`python3 <skill-dir>/blocks.py report <OWNER/REPO> <N> brief` — e.g.
`~/.claude/skills/reports/PROD-7715-agorize-front-3334-brief`. The name starts with the ticket read
from the PR's head branch (`PROD-XXXX-…` → `PROD-XXXX`, `3068-…` or `#3068-…` → `GH-3068`, no
ticket → no prefix), so every page of a ticket, front and core, sorts together; never build the
name by hand. `mkdir -p` the directory first; keep the file: it allows a re-render without redoing
the analysis.

**Build it with a script, never by hand.** Write `<scratch>/build_<N>.py`, run it from the clone
...
```python
import os, sys; sys.path.insert(0, "<skill-dir>")
...
B.hunks("src/x.vue")                               # [(index, "@@ … @@")] — pick `hunks=[…]` from it
B.line("src/x.vue", "  fetchXxx() {")              # a line number, located by pattern
B.excerpt("src/x.vue", "  fetchXxx() {", "^  },", highlight=[".then("])   # ctx by patterns
```

**Never type a line number**: locate it with `B.line` / `B.span` / `B.excerpt`. A pattern starting
with `^` must equal the whole line — `"^  },"` stops at the end of the method, where `"},"` would
stop at the first nested object. List a file's hunks with `B.hunks(path)` (or
`blocks.py hunks <diff> <path>`) before choosing indexes; `blocks.py hunks <diff>` lists the files.
A renamed file is indexed under its **new** path.

**Check every `ran` output against its text** before writing the JSON: a pattern that also
matches a prop declaration or a comment turns a true fact into a false one. Narrow the pattern
(`-E 'canContactAdmins: (true|false)'`), don't drop the output.

Put the helpers a PR needs (paths, `excerpt` wrappers) in the builder script, but keep the script
importable if `pr-flow` may follow: name it `build_<N>.py` and guard the write with
...
On **agorize-front** (Vue 3 Options API, Pinia, Vitest), the usual reach is:
- consumers of a modified Pinia getter, action or state field — `mapStores` gives no import to
  follow, so grep the property name (`xxxStore.someGetter`) across `src`;
- parents of a component whose props, emits or slots changed — grep the component's tag and its
  import, including `defineAsyncComponent` registrations;
- shared mocks under `tests/helpers/mocks/` that other specs import;
- routes and navigation: `src/**/router*`, `navItems`, legacy redirections, route guards;
- `IXxxAttributes` interfaces that mirror an agorize-core serializer — show the serializer on the
  companion branch when one exists (see below);
- shared building blocks (`Base*` components, mixins, SCSS variables and mixins) whose change reaches
  every consumer.

### Companion agorize-core branch — translations and API contract

A front PR usually shows `$translate('spa.x.y')` with no wording: the keys live in agorize-core
(`config/locales/en.yml` and `fr.yml`). Without them, the labels on the page are opaque.

Find the companion branch: a `## Related PRs` section in the front PR body (a link, used as a
locator only, not as a claim), otherwise an open agorize-core PR whose head branch carries the same
ticket prefix (`gh pr list --repo Agorize/agorize-core --search "<PROD-XXXX or #N> in:title"`), otherwise
none. Fetch it into a named ref in `~/workspaces/agorize/agorize-core`
(`git fetch origin refs/pull/<M>/head:refs/pr-brief/core-<M>`), save its diff as
`<scratch>/core-<M>.diff`, and build its blocks with a second instance:
```python
C = Blocks(ref="refs/pr-brief/core-<M>", diff_path="<scratch>/core-<M>.diff",
           cwd=os.path.expanduser("~/workspaces/agorize/agorize-core"),
           repo="Agorize/agorize-core", pr_url="https://github.com/Agorize/agorize-core/pull/<M>")
```
With `repo` set, `C.diff(...)` blocks link to the core PR's files and `C.ctx(...)` /
`C.excerpt(...)` blocks to the core branch at its head sha; the page labels them
`Agorize/agorize-core — <path>`. Core code the companion adds goes in `C.diff` blocks, the rest in
`C.ctx` blocks.

**Stacked companion.** The companion may target another feature branch rather than `master`
(`gh pr view <M> --repo Agorize/agorize-core --json baseRefName`). Find the PR of that base
(`gh pr list --repo Agorize/agorize-core --head <baseRefName> --json number`) and name it: code read
on the companion ref but absent from `core-<M>.diff` comes from that base PR — say "servi par la
base #<K>" rather than attributing it to #<M>. Same on the front side when the PR itself is stacked.

- For every translation key the front diff adds or starts using, embed the `en.yml` and `fr.yml`
  lines (`C.diff` when the companion adds them, `C.excerpt` otherwise).
- When the front PR reads new serializer attributes, embed the serializer from the companion branch.
- A key found neither on the companion branch nor on agorize-core `master` is a fact: state it with
  `C.ran(...)` (`git grep -n '<key leaf>' refs/pr-brief/core-<M> -- config/locales/en.yml`), no
  verdict.
- No companion branch → say so in one line, and look the keys up on agorize-core `origin/master`.

Delete the core ref when done, under the same rule as the front ref (only if this run created it).
...
Omit the key entirely when I have no thread on the PR — most PRs. When I am the PR's author, the
same rule applies: my own threads (self-review drafts included), never the reviewers'.
...
On agorize-front: `bun.lock`, SVG and image assets, snapshots, pure formatting. **Specs
(`*.spec.ts`) and mocks (`tests/helpers/mocks/`) are never noise** — they state the contract
the change is built against; show them in the step they illustrate.
...
- **Vue SFCs**: one `.vue` file mixes `<template>`, `<script>` and `<style>`. Select the hunks by
  block (`hunks=[…]`) and show each where it belongs in the reading path, rather than the whole
  file's diff in one step. A reading order that often works: store (state, getters, actions) →
  component script → template → view wiring.
...
It lives under `~/.claude/skills/reports/`, which the skills repo ignores (`.gitignore`): never
write a brief anywhere else in that repo, and never commit it.
