...
class Flow:
    ...
    def render_diff(self, block):
        ...
        if block.get("repo"):  # a diff from another PR (the agorize-core companion)
            href, label = block.get("url"), f"{block['repo']} — {path}"
        else:
            href = f"{self.pr_url}/files#diff-{hashlib.sha256(path.encode()).hexdigest()}" if self.pr_url else None
            label = path
        return (f'<div class="code">{self.file_header(label, "diff", href)}'
    ...
    def render_context(self, block):
        ...
        return (f'<div class="code">{self.file_header(label, "ctx", href, block.get("note"))}'
