...
- **Branch mechanics**: exactly those of `pr-brief`, step 2 — read `<skill-dir>/../pr-brief/SKILL.md`
  ...
  run created.
- **Reuse the brief when it exists** (`<skill-dir>/../pr-brief/blocks.py report <OWNER/REPO> <N> brief` + `.json`, same `head_sha`). Its
...
- **On agorize-front** (Vue 3 Options API, Pinia), the stages are usually *interaction · route →
  ...
  what the code really does: `@click`, `$emit('select', id)`, `xxxStore.fetchXxx()`,
...
- **The agorize-core side**: when the flow crosses the API, find and read the companion
  agorize-core branch as `pr-brief` describes (section "Companion agorize-core branch"), and give
...
- **Label the arrows.** An unlabelled arrow is "related somehow".
  Two edges leaving the same box draw their labels at the same spot: put `"label_at": "end"` on
